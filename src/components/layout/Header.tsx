...
export function Header({ brokerName, onSignOut }: HeaderProps) {
  ...
  return (
    <header className="h-14 border-b border-white/[0.06] flex items-center justify-between px-3 sm:px-6 relative overflow-hidden">
      {/* Brand */}
      <div className="flex items-center gap-3">
        ...
        <span className="hidden sm:inline text-[10px] font-semibold uppercase tracking-[0.15em] text-white/20">Intelligence</span>
      </div>
        ...
        {nameToShow && (
          <>
            <div className="flex items-center gap-2 bg-white/[0.03] border border-white/[0.06] rounded-full px-3 py-1.5">
              ...
              <span className="text-xs text-white/50 font-medium truncate max-w-[80px] sm:max-w-[200px]">{nameToShow}</span>
            </div>
            {onSignOut && (
              <button
                onClick={onSignOut}
